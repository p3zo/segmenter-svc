...
def make_beat_sync_features(y, sr):
    ...
    beat_times = librosa.frames_to_time(
        librosa.util.fix_frames(beats, x_min=0, x_max=C.shape[1]), sr=sr
    )
...
def embed_beats(A_rep, A_loc):
    ...
    R = librosa.segment.recurrence_matrix(
        A_rep, width=REC_WIDTH, mode="affinity", metric="cosine", sym=True
    )
...
def segment_file(filename):
    ...
    embedding = embed_beats(Csync, Msync)  # CQT and MFCCs
    embedding2 = embed_beats(Csync, Csync)  # just CQT

    Cnorm = np.cumsum(embedding**2, axis=1) ** 0.5
    Cnorm2 = np.cumsum(embedding2**2, axis=1) ** 0.5
    ...
    for k in range(1, MAX_K):
        ...
        segmentations2.append(cluster(embedding2, Cnorm2, k, beat_times))

    return reindex(segmentations), reindex(segmentations2)
